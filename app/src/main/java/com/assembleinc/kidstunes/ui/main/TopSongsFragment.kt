...
class TopSongsFragment : Fragment(), SectionsPagerAdapter.OnFragmentPageSelectedListener {

    // region Member variables

    private lateinit var songsAdapter: SongsAdapter
    private var songs: List<MediaBrowserCompat.MediaItem> = ArrayList()
    private var favoritesIndexes: List<String?> = ArrayList()
    ...
    private fun subscribeToProvider() {
        mediaBrowserConnection?.subscribe(MediaProvider.TOP_SONGS_ROOT_ID)
    }

    private fun unsubscribeToProvider() {
        mediaBrowserConnection?.unsubscribe(MediaProvider.TOP_SONGS_ROOT_ID)
    ...
    private fun updateRecyclerView() {
        ...
        songsAdapter.notifyItemRangeChanged(firstVisibleItem, (lastVisibleItem - firstVisibleItem) + 1)
    }
    ...
    private fun handleStateChange(bundle: Bundle?, playbackState: Int) {
        bundle?.let {
            ...
            isCurrentPlaylist = currentPlaylist == MediaProvider.TOP_SONGS_ROOT_ID
    ...
    private inner class SongsAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
                        ...
                        mediaBrowserConnection?.let {
                            it.mediaController?.let { mc ->
                                val bundle = Bundle().apply {
                                    putString(MediaSessionManager.EXTRA_QUEUE_IDENTIFIER, MediaProvider.TOP_SONGS_ROOT_ID)
                                }
        ...
        private fun bindSongViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            bindFavoriteButton(holder, position)
        ...
        private fun bindFavoriteButton(holder: SongViewHolder, position: Int) {
            val song = songs[position]

            if(favoritesIndexes.contains(song.mediaId)) {
    ...
    private inner class MediaBrowserConnection(context: Context) : MediaBrowserController(context) {
        ...
        override fun onChildrenLoaded(parentId: String, children: List<MediaBrowserCompat.MediaItem>) {
            if (parentId == MediaProvider.TOP_SONGS_ROOT_ID) {
                songs = children
                songsAdapter.notifyDataSetChanged()
                mediaBrowserConnection?.subscribe(MediaProvider.FAVORITES_ROOT_ID)
            }
            else if (parentId == MediaProvider.FAVORITES_ROOT_ID) {
                favoritesIndexes = children.map { it.mediaId }
                songsAdapter.notifyDataSetChanged()
                swipeRefreshLayout?.isRefreshing = false
            }
        }
    ...
    companion object {
        private val TAG = TopSongsFragment::class.java.simpleName
